...
export async function getProjects() {
  ...
  const projects = await Promise.all(data.files.map(f => buildProject(f.id, f.name)));
...
async function buildProject(folderId, folderName) {
  ...
  // Archivos de historia (todo excepto info.txt), ordenados por nombre
  const storyFiles = files
    .filter(f => f.name !== 'info.txt')
    .sort((a, b) => a.name.localeCompare(b.name));

  // Construir bloques de historia: fotos agrupadas + texto intercalado
  const story = [];
  let photoBuffer = [];
